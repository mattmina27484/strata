...
function Sidebar({ route, setRoute }) {
  const Link = ({ id, icon: Icon, label, badge, group = "main" }) => (
    <button
      className={"nav-item " + (route.id === id && !route.assetId ? "active" : "")}
      onClick={() => setRoute({id})}
    >
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="nav-group">
        ...
        <Link id="assets" icon={I.Layers} label="All holdings" badge={(window.ASSETS||[]).length || undefined}/>
        <Link id="categories" icon={I.Home} label="Categories" badge={(window.CATEGORY_TOTALS||[]).filter(c=>c.count>0).length || undefined}/>
...
function App() {
  ...
  const renderScreen = () => {
    if (route.id === "dashboard") {
      ...
      return <DashboardHero range={range} setRange={setRange} onOpenAsset={openAsset} onAdd={() => setRoute({id:"add"})}/>;
    }
